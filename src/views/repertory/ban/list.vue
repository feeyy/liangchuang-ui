<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true">
      <el-form-item label="模板规格" prop="artName">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['repertory:ban:update']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['repertory:ban:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="banProductList" @sort-change='sortChange' @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="本期入库件数" prop="thisInRepertoryNumber" width="140" sortable='custom'/>
      <el-table-column label="本期入库面积" prop="thisInRepertoryArea" width="140" sortable='custom'/>
      <el-table-column label="本期出库件数" prop="thisOutRepertoryNumber" width="140" sortable='custom'/>
      <el-table-column label="本期出库面积" prop="thisOutRepertoryArea" width="140" sortable='custom'/>
      <el-table-column label="期末库存件数" prop="endRepertoryNumber" width="140" sortable='custom'/>
      <el-table-column label="期末库存面积" prop="endRepertoryArea" width="140" sortable='custom'/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="650px" append-to-body center>
      <el-form ref="form" :model="form" label-width="130px">
        <el-form-item label="模板规格" prop="artName" label-width="130px">
          <el-input v-model="form.artName" placeholder="请输入模板规格" />
        </el-form-item>
        <el-form-item label="期初库存数量" prop="earlyRepertoryNumber" label-width="130px">
          <el-input v-model="form.earlyRepertoryNumber" placeholder="请输入期初库存数量" oninput="value=value.replace(/[^0-9]/g,'')" />
        </el-form-item>
...
<script>
  ...
  // 半成品进销存
  import {list, get, add, update, remove} from '@/api/repertory/ban'
    ...
    methods: {
      ...
      list() {
        ...
        list(this.queryParams).then(
          response => {
            this.banProductList = response.rows;
      ...
      handleExport() {
        this.$confirm('您确定要导出当页数据吗？', "警告", {
          ...
          import('../../../utils/Export2Excel').then((excel) => {
            const multiHeader = [['半成品进销存报表', '', '', '', '', '', '', '', '', '']];
            const tHeader = [
              '序号', '模板规格', '期初库存数量', '期初库存面积', '本期入库件数',
              '本期入库面积', '本期出库件数', '本期出库面积', '期末库存件数', '期末库存面积']
            const filterVal = [
              'id', 'artName', 'earlyRepertoryNumber', 'earlyRepertoryArea', 'thisInRepertoryNumber',
              'thisInRepertoryArea', 'thisOutRepertoryNumber', 'thisOutRepertoryArea', 'endRepertoryNumber', 'endRepertoryArea']
            const list = this.banProductList;
            ...
            excel.export_json_to_excel({
              ...
              data,
              filename: '半成品进销存报表' + fileNameDate,
              merges,
            });
          });
        }).catch(function() {
